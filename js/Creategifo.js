/**Variables**/
const begin_btn = document.querySelector(".Comenzar")
const record_btn = document.querySelector(".Grabar")
const end_btn = document.querySelector(".Finzalizar")
const upload_btn = document.querySelector(".Subir")
const step_first = document.getElementById("b1")
const step_second = document.getElementById("b2")
const step_third = document.getElementById("b3")
const counter_recording = document.querySelector(".contadorGrabando")
const counter_repeat = document.querySelector(".contadorRepetir")
const upload_title = document.getElementById("TituloUp")
const upload_text = document.getElementById("TituloTex")
...
const MygifImg=document.querySelector(".MiGifImg")

let recorder;
let blob;
let dateStarted;
let form = new FormData();
let myGifosArray = localStorage.getItem("myGifos")? localStorage.getItem("myGifos").split(","):[];
...
TraerMigif= async ()=>{
    var gifs = await fetch(`https://api.giphy.com/v1/gifs?api_key=${API_KEY}&ids=${myGifosArray.join(",")}`)
    let json= await gifs.json()
    return json.data
}

const MostrarMigif = gifs =>{ 
    if(myGifosArray.length !== 0){
        MygifImg.classList.add ("hide")
    }
    gifs.forEach(gif=>{      
        ...
        MyGifimpresion.appendChild(div)
        MyGifimpresion.classList.add("impresion")
...
TraerMigif().then(response => MostrarMigif(response))